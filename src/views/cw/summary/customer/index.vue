<template>
	<div>
			...
			<div style='flex:1'>
				<el-form ref='formQuery' :model='queryParams' class='c-form-small' inline size='mini'>
					<el-form-item prop='customer_name' label='客户名称'>
						<el-input v-model='queryParams.customer_name' placeholder='客户名称' clearable></el-input>
					</el-form-item>
					<el-form-item prop='customer_name' label='客户类别'>
						<el-input v-model='queryParams.customer_type' placeholder='客户类别' clearable></el-input>
					</el-form-item>
		...
		<el-table 
			...
			<el-table-column prop='customer_name' label='客户名称' fixed width='120'show-overflow-tooltip></el-table-column>
			<el-table-column prop='customer_type' label='客户类别' width='80'></el-table-column>
			<el-table-column prop='project_total' label='项目数量' width='80' align='center'>
				<template slot-scope='{row}'>
					<span class='c-link' @click='openProjectList(row.customer_id)'>{{ row.project_total }}</span>
				</template>
			</el-table-column>
			<el-table-column 
			...
			<el-table-column 
				prop='total_invoice_price' label='已开票金额' min-width='120' align='right'>
				<template slot-scope='{row}'>
...
export default {
	...
	methods:{
		...
		getData() {
			this.loading=true
			cwSummaryApi.getCustomerSummaryList(this.requestParams).then(res=>{
		...
		exportExcel(){
			cwSummaryApi.exportExcelCustomerSummary(this.requestParams)
		}
	}
}
</script>