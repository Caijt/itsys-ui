<template>
	<div>
		<el-alert
	    title="以项目为单位汇总发货记录情况，点击发货记录可查看明细"
	    type="warning" 
	    :closable="false"
		...
		<el-table 
			...
			<el-table-column prop='first_storage_date' label='首次入库日期' align='center' width='110'></el-table-column>
			<el-table-column prop='storage_area' label='累计入库面积' align='right' width='110'>
				<template slot-scope='scope'>
					<span>{{ scope.row.storage_area}}㎡</span>
				</template>
			</el-table-column>
			<el-table-column prop='last_storage_date' label='最近入库日期' align='center' width='110'>
			</el-table-column>
			...
			<el-table-column prop='last_delivery_date' label='最近发货日期' align='center' width='110'></el-table-column>			
			...
			<el-table-column label='操作' fixed='right' align='center' width='90'>
				<template slot-scope='scope'>
					<el-button  size='mini' type='text' @click='orderList(scope)'>发货记录[{{scope.row.record_total}}]</el-button>	
				</template>
			</el-table-column>
		</el-table>
...
export default {
	...
	methods:{
		...
		getData() {
			this.loading=true
			deliveryApi.getSummaryList(this.requestParams).then(res=>{
