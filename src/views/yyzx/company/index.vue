<template>
	<div>
  	<el-row>
			<el-col :span='4'>
				<el-button type='primary' size='small' @click='create'>创建公司</el-button>
			</el-col>
  		...
  		max-height='500'>
			...
			<el-table-column label='操作' fixed='right' align='center' width='120'>
				<template slot-scope='scope'>
					...
					<el-tooltip content='如果业绩公司已关联其它数据（如项目），将无法删除！'>
    ...
    <el-dialog 
    	:title='isFormEdit?"业绩公司修改":"业绩创建公司"' 
...
export default {
	...
	data(){
		return {
			...
			form:{ ...formInit },
			formRules:{
				name:[{	required:true,message:'请填写业绩公司名称' }]
			},
			formLoading:false,
			projectList:[],
			//查询字段
	...
	methods:{		
		...
		getData() {
			this.loading=true
			companyApi.getList(this.requestParams).then(res=>{
		...
		save() {
			this.$refs.form.validate(vaild=>{
				if(vaild){
					...
					if(this.isFormEdit){						
						companyApi.update(this.form).then(res=>{
							this.dialogShow = false
							this.$message.success('修改成功')
							this.getData()				
						})
					}else{						
						companyApi.create(this.form).then(res=>{
							this.dialogShow = false
		...
		del({row,$index}){
			...
			this.$confirm(confirmText,'提示',{
				...
				if(res.code==1){
					...
					companyApi.del(row.id).then(res=>{
						if(res.code==1){
							this.$message.error(res.message)
						}else{
