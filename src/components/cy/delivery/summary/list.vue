  ...
  <div>
    ...
    <div style='margin-bottom: 2px' v-show='!hideQuery'>
      ...
      <el-form ref='formQuery' :model='queryParams' label-width='68px' 
        ...
        <el-form-item prop='toDelivery'>
          <el-tooltip content='待发货的订单（成品库存面积大于发货面积）'>
            <el-checkbox 
              v-model="queryParams.toDelivery" 
              label="待发货" 
              border
              :true-label='1'
              :false-label='0'
              @change='query({key:"toDelivery"})'
              >
            </el-checkbox>
          </el-tooltip>
        </el-form-item>
    ...
    <el-table 
      ...
      <el-table-column prop='order_no' fixed label='订单编号' width='130' show-overflow-tooltip>
       ...
       <el-table-column prop='storage_area_total' label='成品库存面积' sortable='custom' align='right' width='130'>
        <template slot-scope='{ row }'>
          <span>{{ row.storage_area_total }}㎡</span>
        </template>
      </el-table-column>
      <el-table-column prop='delivery_area_total' label='已发货面积' sortable='custom' width='120' align='right'>
        <template slot-scope='{row}'>
          {{ row.delivery_area_total }}㎡
        </template>
      </el-table-column>
      <el-table-column prop='first_delivery_date' label='首次发货日期' sortable='custom' width='120'></el-table-column>
      <el-table-column prop='cy_delivery_record_total' label='发货记录' sortable='custom' width='100' align='center'>
        <template slot-scope='{row}'>
          <span class='c-link' @click='$refs.deliveryListDialog.open({order_id:row.order_id})'>{{ row.cy_delivery_record_total }}</span>
        </template>
      </el-table-column>
      <el-table-column prop='no_delivery_area' label='未发货面积' sortable='custom' width='120' align='right'>
        <template slot-scope='{row}'>
          {{ row.no_delivery_area }}㎡
        </template>
      </el-table-column>
      <el-table-column label='发货进度' min-width='120' align='center'>
        <template slot-scope='{ row }'>
          <el-progress 
            ...
            :status='$commonJs.getProgressPercentage(row.delivery_area_total/row.storage_area_total)>=100?"success":""' 
            :percentage="$commonJs.getProgressPercentage(row.delivery_area_total/row.storage_area_total)"/>
        </template>
      </el-table-column>
    ...
    <delivery-list-dialog :in-dialog='inDialog' ref='deliveryListDialog' />
  </div>
</template>
<script>
...
export default {
  components:{ projectDetails, orderInfo, deliveryListDialog  },
  props:{
    size:{
      type:String,
      default:''
  ...
  methods:{
    ...
    getData() {
      this.loading=true
      deliveryApi.getSummaryListByOrder(this.requestParams).then(res=>{
