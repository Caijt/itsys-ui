...
<template>
	<div>
				...
				<template v-if='queryShowMore'>
					...
					<el-form-item label='入库日期'>
						<el-row style='width:300px'>
							<el-col :span="11">
								<el-form-item prop='storage_date_begin'>
					      	<el-date-picker v-model='queryParams.storage_date_begin' placeholder='开始日期' value-format='yyyy-MM-dd' style='width: 100%'></el-date-picker>
					    	</el-form-item>
					    </el-col>
					    <el-col :span="2">至</el-col>
					    <el-col :span="11">
					    	<el-form-item prop='storage_date_end'>
					    		<el-date-picker v-model='queryParams.storage_date_end' placeholder='结束日期' value-format='yyyy-MM-dd' style='width: 100%'></el-date-picker>
					      </el-form-item>
					    </el-col>
				  	</el-row>
					</el-form-item>
		...
		<el-table 
			...
			<el-table-column prop='order_no' v-if='!hideOrderField' fixed label='订单编号' width='130' show-overflow-tooltip>
				<template slot-scope='{ row }'>
          <span class='c-link' @click='openOrderInfo(row.order_id)'>{{ row.order_no }}</span>
        </template>
			</el-table-column>
			<el-table-column prop='storage_area' label='入库面积' sortable='custom' width='110' align='right'>
				<template slot-scope='{row}'>
					{{ row.storage_area }}㎡
				</template>
			</el-table-column>
			<el-table-column prop='storage_date' label='入库日期' sortable='custom' width='110'></el-table-column>
			<el-table-column v-if='!hideOrderField' prop='order_type' label='订单类别' show-overflow-tooltip></el-table-column>
			<el-table-column v-if='!hideOrderField' prop='order_batch' label='订单批次' show-overflow-tooltip></el-table-column>
			<el-table-column v-if='!hideOrderField' prop='order_area' label='订单面积' sortable='custom' align='right' width='110'>
				<template slot-scope='scope'>
					<span>{{ scope.row.order_area }}㎡</span>
				</template>
			</el-table-column>
			<el-table-column v-if='!hideOrderField' prop='order_amount' label='订单数量' sortable='custom' align='right' width='110'>
			</el-table-column>
			<el-table-column v-if='!hideOrderField' prop='order_date' label='订单日期' sortable='custom' width='110'></el-table-column>
			<el-table-column v-if='!hideProjectField' prop='project_no' label='项目编号' width='100'>
				<template slot-scope='scope'>
					<span @click='openProjectDetails(scope.row.project_id)' class='c-link'>{{ scope.row.project_no }}</span>					
				</template>
			</el-table-column>
			<el-table-column v-if='!hideProjectField' prop='project_name' label='项目名称' width='110' show-overflow-tooltip></el-table-column>
