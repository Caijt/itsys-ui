...
<template>
	<div>
		...
		<div style='margin-bottom: 3px'>
			...
			<el-form ref='formQuery' :model='queryParams' label-width='68px' 
				...
				<div v-show='queryShowMore'>
					...
					<el-form-item label='录入状态' prop='status'>
		...
		<el-table 
			...
			<el-table-column prop='no' fixed label='发货编号' width='120' show-overflow-tooltip>
				<template slot-scope='{row}'>
					{{ row.no }}
				</template>
			</el-table-column>
			<el-table-column prop='status' label='录入状态' width='80' align='center'>
				<template slot-scope='{row}'>
					<el-tag size='mini' type='info' v-show='row.status==0'>未提交</el-tag>
					<el-tag size='mini' type='success' v-show='row.status==1'>已提交</el-tag>
					<el-tag size='mini' type='danger' v-show='row.status==2'>被撤回</el-tag>
				</template>
			</el-table-column>
			...
			<el-table-column prop='order_no' label='订单号' width='120' show-overflow-tooltip></el-table-column>
			...
			<el-table-column prop='maker' label='录入员' ></el-table-column>
...
export default {
	...
	methods:{
		...
		getData() {
			this.loading=true
			deliveryTaskApi.getList(this.requestParams).then(res=>{
		...
		del({ row,$index }){
			...
			this.$confirm(confirmText,'提示',{
				...
				deliveryTaskApi.del(row.id).then(res=>{
