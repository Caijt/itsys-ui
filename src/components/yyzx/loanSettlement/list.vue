...
<template>
  <div>
    ...
    <el-table 
      ...
      <el-table-column prop='contract_no' label='回款合同' width='100' show-overflow-tooltip />
...
export default {
  ...
  methods:{
    ...
    getData() {
      this.loading=true
      paymentApi.getList({...this.requestParams,...this.initParams}).then(res=>{
    ...
    undo({ row,$index }){
      let confirmText = '确定撤销此回款吗？撤销后将恢复到编辑列表中'
