	...
	<div>
		...
		<div style='margin-bottom: 3px'>
			...
			<el-form ref='formQuery' :model='queryParams' label-width='68px' 
				...
				<el-form-item prop='noOrder'>
					<el-tooltip content='只显示未下过单的项目'>
						<el-checkbox 
							...
							:true-label='1'
							:false-label='0'
							@change='query({key:"noOrder"})'
							>
						</el-checkbox>
					</el-tooltip>
				</el-form-item>
				<el-form-item prop='toOrder'>
					<el-tooltip content='只显示待下单项目（下单面积小于项目面积）'>
						<el-checkbox 
...
export default {
	components:{ projectDetails, orderDialog, contractInfo },
	props:{
		size:{
			type:String,
			default:''
	...
	methods:{
		...
		openOrderDialog(project_id){
			this.$refs.orderDialog.open( { project_id,isDel:0 })
		}
	}
}
</script>