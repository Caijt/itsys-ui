...
<template>
	<div>
		...
		<el-table 
			...
			<el-table-column prop='order_amount' label='下单数量' sortable='custom' align='right' width='110'></el-table-column>			
			<el-table-column prop='is_contract' width='100' label='合同下单' sortable='custom' align='center'>
				<template slot-scope='{row}'>
					<el-tag size='mini' type="success" v-if='row.is_contract==1'>有</el-tag>
...
export default {
	...
	methods:{
		...
		getData() {
			this.loading=true
			orderApi.getList(this.requestParams).then(res=>{
