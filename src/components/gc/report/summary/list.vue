	...
	<div>
		...
		<div style='margin-bottom:3px '>
			...
			<el-form ref='formQuery' :model='queryParams' label-width='68px' 
				...
				<el-form-item prop='isGcNew'>
					<el-tooltip content='未填写报告的新项目'>
						<el-checkbox 
							v-model="queryParams.isGcNew" 
							label="新项目" 
							border
							:true-label='1'
							:false-label='0'
							@change='query({key:"isGcNew"})'
							>
						</el-checkbox>
					</el-tooltip>
				</el-form-item>
				...
				<template v-if='queryShowMore'>
					...
					<el-form-item label='管理人员' prop='project_admin'>
						<el-input v-model='queryParams.project_admin' clearable></el-input>
					</el-form-item>
					<el-form-item label='安装计划' prop='project_install_plan'>
						<el-input v-model='queryParams.project_install_plan' clearable></el-input>
					</el-form-item>
					<el-form-item label='提供安装' prop='project_is_install'>
						<el-select v-model='queryParams.project_is_install' clearable>
							<el-option :value='1' label='是'></el-option>
							<el-option :value='0' label='否'></el-option>
						</el-select>
					</el-form-item>	
					<el-form-item label='安装进度' prop='project_install_progress'>
						<el-input v-model='queryParams.project_install_progress' clearable></el-input>
					</el-form-item>
					<el-form-item label='劳务合同' prop='project_contract_status'>
						<el-input v-model='queryParams.project_contract_status' placeholder='劳务合同执行情况' clearable></el-input>
					</el-form-item>
				</template>
				<!--/ 更多条件-->
			</el-form>
		...
		<el-table 
			...
			<el-table-column prop='engineering_is_install' label='提供安装' width='80'>
				<template slot-scope='{row}'>
					{{ row.gc_update_time?(row.engineering_is_install?'是':'否'):''}}
				</template>
			</el-table-column>
			<el-table-column prop='engineering_admin' label='项目管理人员' show-overflow-tooltip min-width='150'></el-table-column>
			<el-table-column prop='engineering_install_plan' label='安装计划' show-overflow-tooltip min-width='150'></el-table-column>
			<el-table-column prop='engineering_install_progress' label='安装进度' show-overflow-tooltip min-width='150'></el-table-column>
			<el-table-column prop='engineering_contract_status' label='劳务合同执行情况' show-overflow-tooltip min-width='150'></el-table-column>
			<el-table-column prop='gc_update_time' label='报告更新时间' width='135' sortable='custom'>
				<template slot-scope='{ row }'>
					<span>{{row.gc_update_time?$moment(row.gc_update_time).format('YYYY-MM-DD HH:mm'):"（未填写）"}}</span>
				</template>
			</el-table-column>
			<el-table-column prop='contract_no' label='合同编号' sortable='custom' width='100' show-overflow-tooltip>
				<template slot-scope='{row}'>
			...
			<el-table-column prop='project_create_time' label='项目创建时间' width='135' sortable='custom'>
				<template slot-scope='{ row }'>
					<span>{{ $moment(row.project_create_time).format('YYYY-MM-DD HH:mm') }}</span>
				</template>
			</el-table-column>
			<!-- slot[column] -->
...
<script>
import projectApi from '@/api/gc/report'
