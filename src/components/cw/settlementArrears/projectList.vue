...
<template>
  <div>
    ...
    <div style='margin-bottom:2px' v-show='!hideQuery'>
      ...
      <el-form ref='formQuery' :model='queryParams' label-width='68px' 
        ...
        <el-form-item prop='isArrears'>
          <el-tooltip content='只显示在途物资金额不等于0的项目'>
            <el-checkbox 
    ...
    <el-table 
      ...
      <el-table-column prop='settlement_price_total' sortable='custom' align='right' label='已对账货款' width='120'>
        <template slot-scope='{row}'>
      ...
      <el-table-column 
        ...
        sortable='custom'>
        <template slot-scope='{ row }'>           
          <el-tooltip placement='left' :content='row.cw_invoice_price_total+"㎡（已开票金额） / "+row.settlement_price_total+"㎡（已对账金额）"'>
            <el-progress 
              text-inside 
              :stroke-width='16' 
      ...
      <el-table-column prop='cw_invoice_record_total' label='开票记录' sortable='custom' width='100' align='center'>
        <template slot-scope='{row}'>
          <span class='c-link' @click='openRecord(row)'>{{row.cw_invoice_record_total}}</span>
        </template>
      </el-table-column>
...
import invoiceDialog from '@/components/cw/invoiceRecord/dialog'
import contractInfo from '@/components/yyzx/contract/info'

export default {
  components:{ invoiceDialog, contractInfo },
  props:{
    size:{
      type:String,
      default:''
  ...
  methods:{
    ...
    openRecord(row){
      this.$refs.invoiceDialog.open({project_id:row.id})
    },
    exportExcel(){
