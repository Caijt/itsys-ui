...
<script>
import paymentApi from '@/api/cw/receivePaymentProject'
...
export default {
  ...
  methods:{
    ...
    getData() {
      this.loading=true
      paymentApi.getCompanyStatisticList({...this.requestParams,...this.initParams}).then(res=>{
    ...
    openPaymentDialog(row){
      this.$refs.paymentDialog.open().then(that=>{
        that.initData({ company_id:row.company_id })
      })
    }
  }
}
