...
<template>
  <div>
    ...
    <el-table 
      ...
      <el-table-column prop='project_no' fixed label='项目编号' width='110'>
        <template slot-scope='{row}'>
          <span class='c-link' @click='openProjectDetails(row)'>{{row.project_no}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop='company_name' label='业绩公司' width='100' show-overflow-tooltip />
      <el-table-column prop='salesman' label='业务员' width='100' show-overflow-tooltip />
      <el-table-column prop='place' label='项目地点' width='100' show-overflow-tooltip />
      <el-table-column prop='payment_price_total' align='right' sortable='custom' label='已回款金额' width='110' >
        <template slot-scope='{row}'>
          ￥{{ row.payment_price_total }}
        </template>
      </el-table-column>
      <el-table-column prop='payment_total' label='回款记录' align='center' sortable='custom' width='100'>
        <template slot-scope='{row}'>
          <span class='c-link' @click='openPaymentDialog(row)'>{{row.payment_total}}</span>
        </template>        
      </el-table-column>
      <el-table-column prop='last_payment_date' label='最近回款日期' sortable='custom' width='120' show-overflow-tooltip />      
      <!-- slot[column] -->
    ...
    <!--/ 分页 -->  
    <payment-dialog :in-dialog='inDialog' ref='paymentDialog'/>
  </div>
</template>
<script>
import receivePaymentApi from '@/api/cw/receivePaymentProject'
import paymentDialog from '../paymentProject/listDialog'
...
export default {
  ...
  methods:{
    ...
    getData() {
      this.loading=true
      receivePaymentApi.getProjectStatisticList({...this.requestParams,...this.initParams}).then(res=>{
    ...
    exportExcel(){
      receivePaymentApi.exportExcel(this.requestParams)
    },
    ...
    openPaymentDialog(row){
      this.$refs.paymentDialog.open().then(that=>{
        that.initData({project_id:row.project_id})
      })
    }
  }
}
