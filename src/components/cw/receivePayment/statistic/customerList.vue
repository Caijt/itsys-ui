...
<template>
  <div>
    ...
    <el-table 
      ...
      <el-table-column prop='customer_name' fixed label='客户名称' min-width='130' show-overflow-tooltip />
      <el-table-column prop='customer_type' label='客户类型' width='150' show-overflow-tooltip />
      <el-table-column prop='payment_price_total' align='right' sortable='custom' label='回款金额' width='110' >
...
import paymentDialog from '../edit/listDialog'
...
export default {
  ...
  methods:{
    ...
    getData() {
      this.loading=true
      paymentApi.getCustomerStatisticList({...this.requestParams,...this.initParams}).then(res=>{
    ...
    openPaymentDialog(row){
      this.$refs.paymentDialog.open().then(that=>{
        that.initData({customer_id:row.customer_id})
      })
    }
  }
}
