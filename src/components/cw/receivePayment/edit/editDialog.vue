	...
	<el-dialog 
		...
		<div v-loading='loading'>
			...
			<el-form 
				...
				class='c-form-mini'>
				<divider title='本次回款信息'></divider>	
				<el-row :gutter='10'>
					<el-col :span='16'>
						<el-form-item label='回款客户' prop='customer_id' class='no-border' :inline-message='true'>
							<el-button type='primary' @click='openCustomerDialog'>{{ form.customer_id?form.customer_name:'选择'}}</el-button>
						</el-form-item>
					</el-col>
				</el-row>
				<el-row :gutter='10'>
					<el-col :span='8'>
						<el-form-item label='回款合同' prop='contract_no'>
						  <el-input v-model='form.contract_no'>
						  	<el-button slot='append' @click='openContract'>提取</el-button>
							</el-input>
						</el-form-item>
					</el-col>
					<el-col :span='8'>
					...
					<el-col :span='8'>
						<el-form-item label='回款形式' prop='payment_method'>
						  <el-select v-model='form.payment_method' style='width: 100%'>
						  	<el-option v-for='m in paymentMethodList' :value='m' :key='m'></el-option>
						  </el-select>
						</el-form-item>
					</el-col>
				</el-row>
		...
		<customer-list-dialog ref='customerListDialog'>
			<el-table-column slot='column' fixed='right' align='center' label='操作' width='60'>
				<template slot-scope='{row}'>
					<el-button type='text' @click='selectCustomer(row)'>选择</el-button>
				</template>
			</el-table-column>
		</customer-list-dialog>
		<contract-list-dialog ref='contractListDialog'>
			<el-table-column slot='column' fixed='right' align='center' label='操作' width='60'>
				<template slot-scope='{row}'>
					<el-button type='text' @click='selectContract(row)'>选择</el-button>
				</template>
			</el-table-column>
...
<script>
	...
	import customerListDialog from '@/components/cw/arrears/customerListDialog'
	...
	export default {
		...
		computed:{
			...
			title(){
				let title = this.isEdit?'回款编辑':'回款录入'
				return title
			},
			projectPriceTotal(){
				let sum = 0
