...
<template>
	<div>
			 ...
			 <div style='float:right;margin-top: 1px'>
				...
				<el-form ref='formQuery' :model='queryParams' label-width='68px' 
					...
					<el-form-item prop='hasArrear'>
						<el-tooltip content='只显示待回款项目（回款金额小于开票目金额）'>
							<el-checkbox 
		...
		<el-table 
			...
			<el-table-column prop='project_price' label='项目金额' min-width='120' align='right'>
				<template slot-scope='scope'>
					<span>￥{{ scope.row.project_price }}</span>
				</template>
			</el-table-column>
			<el-table-column 
				prop='cw_invoice_price_total' label='已开票金额' width='120' align='right' sortable='custom'>
				<template slot-scope='{ row }'>
					<span>￥{{ row.cw_invoice_price_total }}</span>
				</template>
			</el-table-column>
			<el-table-column 
				prop='cw_payment_price_total' label='已回款金额' width='120' align='right' sortable='custom'>
				<template slot-scope='{ row }'>
					<span>￥{{ row.cw_payment_price_total }}</span>
				</template>
			</el-table-column>
			<el-table-column prop='payment_record_total' align='center' label='回款记录'>
				<template slot-scope='{row}'>
					<span class='c-link' @click='openInvoiceRecord(row.project_id)'>{{ row.payment_record_total }}</span>
				</template>
			</el-table-column>
			<el-table-column 
				prop='no_payment_price' label='应收欠款金额' width='120' align='right' sortable='custom'>
				<template slot-scope='{ row }'>
					<span>￥{{ row.no_payment_price }}</span>
				</template>
			</el-table-column>
			<el-table-column label='项目回款率' min-width='120' align='center'>
				<template slot-scope='{ row }'>
					<el-progress 
			...
			<el-table-column 
				prop='last_payment_date' label='最近回款日期' width='130' sortable='custom'/>
...
export default {
	...
	methods:{
		...
		getData() {
			this.loading=true
			receivePaymentRecordApi.getSummaryListByProject(this.requestParams).then(res=>{
