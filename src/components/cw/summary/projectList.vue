...
<template>
	<div>
		...
		<div style='margin-bottom: 3px'>
			...
			<el-form ref='formQuery' :model='queryParams' label-width='68px' 
				...
				<el-form-item prop='hasDelivery'>
					<el-tooltip content='只显示已发货的项目（发货面积大于0）'>
						<el-checkbox 
				...
				<el-form-item prop='hasArrear'>
					<el-tooltip content='只显示欠款项目（回款金额小于开票金额）'>
						<el-checkbox 
		...
		<el-table 
			...
			<el-table-column prop='settlement_price_total' label='项目金额' min-width='120' align='right'>
				<template slot-scope='scope'>
			...
			<el-table-column 
				prop='payment_price_total' label='已回款金额' width='120' align='right' sortable='custom'>
				<template slot-scope='scope'>
					<span>￥{{ scope.row.payment_price_total }}</span>
				</template>
			</el-table-column>
			<el-table-column prop='payment_record_total' align='center' label='回款记录'>
				<template slot-scope='{row}'>
					<span class='c-link' @click='openReceivePaymentRecord(row.project_id)'>{{ row.payment_record_total }}</span>
				</template>
			</el-table-column>
			<el-table-column 
				prop='arrear_price' label='应收欠款金额' width='130' align='right' sortable='custom'>
				<template slot-scope='{ row }'>
					<span>￥{{ row.arrear_price }}</span>
				</template>
			</el-table-column>
...
export default {
	components:{ projectDetails, invoiceRecordDialog, receivePaymentRecordDialog },
	props:{
		size:{
			type:String,
			default:''
	...
	methods:{
		...
		getData() {
			this.loading=true
			cwSummaryApi.getProjectSummaryList(this.requestParams).then(res=>{
