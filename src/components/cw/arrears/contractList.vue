...
<template>
  <div>
    ...
    <div style='margin-bottom:2px' v-show='!hideQuery'>
      ...
      <el-form ref='formQuery' :model='queryParams' label-width='68px' 
        ...
        <el-form-item prop='isArrears'>
          <el-tooltip content='只显示应收款不等于0的合同'>
            <el-checkbox 
    ...
    <el-table 
      ...
      <el-table-column label='回款进度' width='120' prop='payment_progress' sortable align='center'>
        <template slot-scope='{ row }'>           
          <el-tooltip placement='left' :content='row.cw_payment_price_total+"㎡（已回款金额） / "+row.contract_price+"㎡（合同款）"'>
            <el-progress 
              text-inside 
              :stroke-width='16' 
