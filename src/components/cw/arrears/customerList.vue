...
<template>
  <div>
    ...
    <div style='margin-bottom:2px' v-show='!hideQuery'>
      ...
      <el-form ref='formQuery' :model='queryParams' label-width='68px' 
        ...
        <el-form-item prop='isArrears'>
          <el-tooltip content='只显示应收款不等于0的客户'>
            <el-checkbox 
    ...
    <el-table 
      ...
      <el-table-column prop='payment_record_total' label='回款记录' sortable='custom' width='100' align='center'>
        <template slot-scope='{row}'>
          <span class='c-link' @click='openPayment(row)'>{{row.payment_record_total}}</span>
        </template>
      </el-table-column>
      <el-table-column prop='last_payment_date' label='最近回款日期' sortable='custom' width='120' />
      <el-table-column prop='invoice_price_total' align='right' sortable='custom' label='已开票金额' width='120' >
        <template slot-scope='{row}'>
          ￥{{ row.invoice_price_total }}
        </template>
      </el-table-column>
      <el-table-column prop='payment_price_total' align='right' sortable='custom' label='已回款金额' width='120' >
        <template slot-scope='{row}'>
