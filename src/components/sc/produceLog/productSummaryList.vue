	...
	<div>
        ...
        <template slot-scope='{row}'>
          <el-progress 
			...
			<el-table-column prop='amount_total' label='产品数量' align='right' width='100' sortable='custom' show-overflow-tooltip>
				<template slot-scope='{row}'>
					{{row.amount_total}} 樘
				</template>
			</el-table-column>
			...
			<el-table-column prop='machining_area_total'  sortable='custom' label='加工面积' width='100' align='right'show-overflow-tooltip>
				<template slot-scope='{row}'>
			...
			<el-table-column prop='sc_produce_log_total' label='生产日志' sortable='custom' align='center' width='100'>
				<template slot-scope='{row}'>
					<span class='c-link' @click='$refs.logListDialog.open({product_ids:row.product_ids})'>{{ row.sc_produce_log_total }}</span>
				</template>
			</el-table-column>
...
export default {
	components:{ orderInfo, productListDialog, logListDialog },
	props:{
		size:{
			type:String,
			default:''
	...
	methods:{
		...
		getData() {
			this.loading=true
			produceLogApi.getProductSummaryList(this.requestParams).then(res=>{
