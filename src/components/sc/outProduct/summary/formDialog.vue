<template>
	...
	<el-dialog 
							...
							<el-row :gutter='10'>
								...
								<el-col :span='24'>
									<el-form-item label='生产状态' prop='sc_out_product_status'>
										<el-input v-model='form.sc_out_product_status' >
										</el-input>
									</el-form-item>
								</el-col>
								<el-col :span='24'>
									<el-form-item label='加工单位' prop='sc_out_product_unit'>
										<el-input v-model='form.sc_out_product_unit' >
										</el-input>
									</el-form-item>
								</el-col>
							</el-row>
		...
		<span slot="footer">
			<el-button type='primary' size='mini' @click='create' :disabled='!form.order_id||form.sc_out_is_finish==1'>添加生产日志</el-button>
	    <el-button type='success' size='mini' @click='productFinish' :disabled='!form.order_id||form.sc_out_is_finish==1'>生产完成</el-button>
	    <el-button type='primary' size='mini' @click='save' :disabled='!form.order_id||form.sc_out_is_finish==1' :loading='buttonLoading'>更新信息</el-button>
...
<script>
	import outProductApi from '@/api/sc/outProduct'
		...
		methods:{
			...
			save(){				
				this.$refs.form.validate(valid=>{
					if(valid){
						...
						outProductApi.updateInfo({
							order_id:this.form.order_id,
							product_status:this.form.sc_out_product_status,
							product_unit:this.form.sc_out_product_unit
						}).then(res=>{
			...
			selectOrder(row){
				let data = this.$commonJs.obj.copyByKey(row,[
					...
					'project_no','contract_no','plan_id','sc_out_product_status','sc_out_product_unit'
				])
				this.assign(data)
			...
			reload(){
				outProductApi.getSummary({order_id:this.form.order_id}).then(res=>{
			...
			productFinish(){
				this.$confirm('当前订单已生产完成？确定后此订单将无法增加或修改生产日志','提示',{
					type: 'warning'
				}).then(()=>{
					outProductApi.productFinish(this.form.order_id).then(res=>{
