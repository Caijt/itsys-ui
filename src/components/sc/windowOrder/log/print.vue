		...
		<div v-loading='loading' style='position: relative;'>
			...
			<div class='_order-print-page'>
							...
							<td colspan="8" style='padding:0px;'>
								<table class='c-table' style='border-style:hidden;'>
									...
									<tr v-for='(a,index) in item.detailsList'>
										...
										<td>{{a.floor_no}}</td>
										<td>{{a.window_no}}</td>									
										<td>{{Number(a.width)}}</td>
										<td>{{Number(a.height)}}</td>
										<td style='text-align: right'>{{a.amount}}</td>										
										<td style='text-align: right'>{{Number(a.area)}}</td>
									</tr>
									<tr>
										<td colspan="5" class='label'>小计</td>
										<td style='text-align: right'>{{ getSummaryTotal(item.detailsList,'amount')}}</td>
										<td style='text-align: right'>{{ getSummaryTotal(item.detailsList,'area')}}</td>
									</tr>
								</table>
							</td>
						</tr>
				...
				<div style='display: flex'>
					<div style='flex:1'>下单人：{{data.create_user_name}}</div>
