	...
	<div>
		<el-form 
			...
			<div style='margin-bottom: 5px'>
				<el-button type='primary' @click='addProduct'>选择产品</el-button>
				<el-tooltip content='对勾选的产品回款金额填充最大值'>
      ...
      <el-table-column prop='payment_price' label='生产数量' width='100' fixed='right'>
				<template slot-scope='{row}'>
					<el-form-item style='margin-bottom: 0px' prop='payment_price' :rules='getPriceRules(row)'>
						<el-input v-model.number='row.amount' :placeholder='row.arrears_price'>
							<span slot="suffix">{{row.unit}}</span>
						</el-input>
					</el-form-item>
				</template>
			</el-table-column>
			<el-table-column prop='payment_price' label='生产面积' width='150' fixed='right'>
				<template slot-scope='{row}'>
					<el-form-item style='margin-bottom: 0px' prop='payment_price' :rules='getPriceRules(row)'>
						<el-input v-model.number='row.area' :placeholder='row.arrears_price'>
      ...
      <!--/ slot[column]-->
		...
		<project-list-dialog :in-dialog='inDialog' hide-order-fields ref='projectListDialog'>
...
<script>
	import paymentProjectApi from '@/api/yyzx/orderProduct'
	import projectListDialog from '@/components/sc/windowOrder/product/listDialog'
