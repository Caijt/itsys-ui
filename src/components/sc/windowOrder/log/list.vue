...
<template>
	<div>
		...
		<el-table 
			...
			<el-table-column prop='machining_area' label='产品面积' align='right' width='100' show-overflow-tooltip>
				<template slot-scope='{row}'>
					{{row.area}} ㎡
				</template>
			</el-table-column>
        ...
        <template slot-scope='{row}'>
          <el-progress 
            ...
            :status='$commonJs.getProgressPercentage(row.sc_produce_area_total/row.area)>=100?"success":""' 
            :percentage="$commonJs.getProgressPercentage(row.sc_produce_area_total/row.area)"/>
        </template>
      </el-table-column>
...
export default {
	...
	methods:{
		...
		getData() {
			this.loading=true
			produceLogApi.getList({...this.requestParams,...this.params,...this.initParams}).then(res=>{
