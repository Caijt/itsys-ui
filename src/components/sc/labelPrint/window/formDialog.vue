		...
		<el-dialog 
			...
			<div v-loading='loading'>
				<el-form :model='form' :rules='rules' ref='form' label-width='100px' size='mini'  status-icon class='c-form-mini'>
					<divider title='门窗标签信息'></divider>
					<el-form-item label='窗号' prop='no'>
					...
					<el-form-item label='生产类型' prop='type'>
						<el-radio-group v-model="form.type">
					    <el-radio label="S">汕头生产（S）</el-radio>
					    <el-radio label="W">吴江生产（W）</el-radio>
					    <el-radio label="X">外协生产（X）</el-radio>
					  </el-radio-group>
					</el-form-item>
...
<script>
	...
	export default {
		...
		computed:{
			...
			title(){
				return this.isEdit?'编辑门窗标签':'创建门窗标签'
			},
			buttonText(){
			...
			save() {
				this.$refs.form.validate(vaild=>{
					if(vaild){
						this.loading = true
						if(this.isEdit){						
							windowApi.update(this.form).then(res=>{
							...
							windowApi.create(this.form).then(res=>{
