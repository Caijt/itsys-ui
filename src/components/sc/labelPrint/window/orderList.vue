...
<script>
import windowOrderApi from '@/api/sc/windowOrder'
import orderInfo from '@/components/yyzx/order/info'
import listDialog from '@/components/sc/labelPrint/window/listDialog'
...
export default {
	...
	methods:{
		...
		getData() {
			this.loading=true
			windowOrderApi.getList(this.requestParams).then(res=>{
    ...
    del({ row,$index }){
			this.$confirm('确定删除？','提示',{
				...
				windowOrderApi.del(row.id).then(res=>{
		...
		exportExcel(){
      windowOrderApi.exportExcel(this.requestParams)
    },
	}
}
</script>