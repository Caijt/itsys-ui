<template>
	...
	<el-dialog 
		...
		<divider title='门窗标签列表'></divider>
		<el-alert title='创建标签后，必须得生成防伪二维码后才能打印' type='warning' style='margin-bottom: 10px'></el-alert>
		<window-list 
			:params='params' ref='windowList' size='mini' no-page hide-order-fields @del='reload' show-summary>
...
<script>
	...
	import windowList from '@/components/sc/labelPrint/window/list'
		...
		methods:{
			...
			qrcode(){
				...
				this.$confirm('确定要生成标签的防伪二维码？生成防伪二维码后标签信息将无法修改及删除！','提示',{
					...
					windowApi.createCode({window_ids:ids}).then(res=>{
