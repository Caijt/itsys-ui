<template>
	<div>
		<el-dialog 
		    ...
		    <el-button type='success' @click="down('sc_window_import_template.xlsx')" size='small'>下载导入模板</el-button>
...
<script>
	...
	export default {
		...
		data(){			
			return {
				attachUploadUrl:windowApi.excelUploadUrl,
				changed:false,
				show:false,
				buttonLoading:false,
				data:null,
		...
		methods:{
			...
			upload(){
				...
				this.$refs.attachUpload.upload().then(res=>{
					this.$message.success('成功导入'+ res +'个门窗标签信息')
					this.changed = true
					this.show = false
					this.loading = false
				}).catch(res=>{
