	...
	<div>
        ...
        <el-tooltip content='显示更多查询条件' placement='top'>
          ...
          <i :class="{'el-icon-arrow-up':queryShowMore,'el-icon-arrow-down':!queryShowMore}"></i>
			...
			<el-form ref='formQuery' :model='queryParams' label-width='68px' 
				...
				<el-form-item label='玻璃编号' prop='glass_no'>
					<el-input v-model='queryParams.glass_no' clearable></el-input>
				</el-form-item>
				<!-- 更多条件 -->
				<template v-if='queryShowMore'>
					<el-form-item label='玻璃型号' prop='glass_model'>
						<el-input v-model='queryParams.glass_model' clearable></el-input>
					</el-form-item>
		...
		<el-table 
          ...
          <span >{{ row.order_no }}</span>
			...
			<el-table-column prop='is_code' label='防伪二维码' align='center' width='90'>
				<template slot-scope='{row}'>
					<el-tag size='mini' type='info' v-if='row.is_code==0'>未生成</el-tag>
					<el-tag size='mini' type='success' v-else>已生成</el-tag>
				</template>
			</el-table-column>
			<el-table-column prop='no' label='玻璃编号' show-overflow-tooltip></el-table-column>
			<el-table-column prop='model' label='玻璃型号' show-overflow-tooltip></el-table-column>
			...
			<el-table-column prop='sn' label='生产序号' sortable='custom' width='100'></el-table-column>
			<el-table-column prop='flow_no' label='流程单号' sortable='custom' width='100' show-overflow-tooltip></el-table-column>
...
export default {
	...
	methods:{
		...
		getData() {
			this.loading=true
			glassApi.getList(this.requestParams).then(res=>{
    ...
    del({ row,$index }){
		...
		exportExcel(){
      glassApi.exportExcel(this.requestParams)
    },
	}
}
</script>