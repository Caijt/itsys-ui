<template>
	<div>
		<el-dialog 
		    ...
		    <el-button type='success' @click="down('sc_certificate_import_template.xlsx')" size='small'>下载导入模板</el-button>
...
<script>
	...
	export default {
		...
		data(){			
			return {
				attachUploadUrl:certificateApi.excelUploadUrl,
				changed:false,
				show:false,
				buttonLoading:false,
				data:null,
		...
		methods:{
			...
			upload(){
				...
				this.$refs.attachUpload.upload().then(res=>{
					this.$message.success('成功导入'+res+'个合格证信息')
					this.changed = true
					this.show = false
					this.loading = false
				}).catch(res=>{
