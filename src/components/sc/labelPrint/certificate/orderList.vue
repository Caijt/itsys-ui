...
import listDialog from '@/components/sc/labelPrint/certificate/listDialog'
...
export default {
	...
	methods:{
		...
		getData() {
			this.loading=true
			certificateOrderApi.getList(this.requestParams).then(res=>{
    ...
    del({ row,$index }){
			this.$confirm('确定删除？','提示',{
				...
				certificateOrderApi.del(row.id).then(res=>{
		...
		exportExcel(){
      certificateOrderApi.exportExcel(this.requestParams)
    },
	}
}
</script>