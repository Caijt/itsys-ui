...
<template>
  <div>
    ...
    <el-table 
      ...
      <el-table-column prop='no' label='打样编号' width='110'>
        <template slot-scope='{row}'>
          <span class='c-link' @click='$refs.progressInfo.open(row.id)'>{{row.no}}[{{row.time}}]</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop='amount' label='色板数量' width='95' align='right' sortable='custom'/>
      ...
      <el-table-column prop='create_user_name' label='销售申请人' width='100'/>
      <el-table-column prop='submit_time' label='申请提交时间' sortable='custom' width='130'>
        <template slot-scope='{row}'>
      ...
      <el-table-column prop='file_user_name' label='归档人' width='80' show-overflow-tooltip />
      <el-table-column prop='file_time' label='归档提交时间' sortable='custom' width='130'>
        <template slot-scope='{row}'>
      ...
      <el-table-column prop='make_user_name' label='采购人' width='80' show-overflow-tooltip />
      <el-table-column prop='make_time' label='打板提交时间' sortable='custom' width='130'>
        <template slot-scope='{row}'>
      ...
      <el-table-column prop='delivery_user_name' label='发出人' width='120' show-overflow-tooltip />
      <el-table-column prop='delivery_time' label='发出提交时间' width='130' sortable='custom' >
        <template slot-scope='{row}'>
      ...
      <el-table-column prop='is_confirm' label='确认结果' align='center' width='80'>
        <template slot-scope='{row}'>
          ...
          <el-tag type='danger' size='mini' v-show='row.is_confirm==0'>不通过</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop='confirm_remarks' label='确认说明' width='100' show-overflow-tooltip />
      <el-table-column prop='confirm_user_name' label='确认人' width='100' show-overflow-tooltip />
      <el-table-column prop='confirm_time' label='确认提交时间' width='120' sortable='custom'>
         <template slot-scope='{row}'>
    ...
    <!--/ 分页 -->   
    <progress-info :in-dialog='inDialog' ref='progressInfo' />
  </div>
</template>
<script>
...
export default {
  ...
  created(){
    this.$options.components.progressInfo = ()=>import('@/components/xs/colorSample/progressInfo')
  },
  methods:{
    initData(){
      this.resetQuery()
    ...
    getData() {
      this.loading=true
      colorSampleApi.getProgressList(this.requestParams).then(res=>{
    ...
    exportExcel(){
      colorSampleApi.exportProgressExcel(this.requestParams)
    },
