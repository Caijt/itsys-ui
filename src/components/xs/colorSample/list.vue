...
<template>
  <div>
    ...
    <el-table 
      ...
      <el-table-column prop='status' label='当前状态' width='120' show-overflow-tooltip>
        <template slot-scope='{row}'>
      ...
      <el-table-column prop='order_create_time' label='创建时间' sortable='custom' width='120'>
        <template slot-scope='{row}'>
          {{ $commonJs.formatDate(row.create_time) }}
        </template>
      </el-table-column>
      <el-table-column prop='order_create_time' label='更新时间' sortable='custom' width='120'>
        <template slot-scope='{row}'>
