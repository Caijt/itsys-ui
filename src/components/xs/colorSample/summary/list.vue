...
<template>
  <div>
    ...
    <el-table 
      ...
      <el-table-column prop='input_status' label='录入状态' width='80' align='center'>
        <template slot-scope='{row}'>
          <el-tag size='mini' type='info' v-if='row.input_status == 0'>暂存</el-tag>
          <el-tag size='mini' type='success' v-else-if='row.input_status == 1'>提交</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column prop='times' label='打样次数' align='center' sortable='custom' width='100'>
        <template slot-scope='{row}'>
          <span class='c-link'>{{ row.times }}</span>
        </template>
      </el-table-column>
      <el-table-column prop='express' label='快递信息' width='120' show-overflow-tooltip />
      <el-table-column prop='create_user_name' label='录入员' />
      <el-table-column prop='order_create_time' label='创建时间' sortable='custom' width='140'>
        <template slot-scope='{row}'>
          {{ $commonJs.formatDate(row.create_time) }}
        </template>
      </el-table-column>
      <el-table-column prop='order_create_time' label='更新时间' sortable='custom' width='140'>
        <template slot-scope='{row}'>
...
<script>
import productApi from '@/api/xs/colorSample'
