...
<template>
  <div>
    ...
    <div style='margin-bottom:2px' v-show='!hideQuery'>
      ...
      <el-form ref='formQuery' :model='queryParams' label-width='68px' 
        ...
        <template v-if='queryShowMore'>
          <el-form-item label='打样状态' prop='status'>
            <el-select v-model='queryParams.status' multiple collapse-tags>
              <el-option value='SUBMIT' label='待打样'></el-option>
          ...
          <el-form-item label='其它要求' prop='other' >
            <el-input v-model.trim='queryParams.other' clearable></el-input>
          </el-form-item>
        </template>
        <!--/ 更多条件-->
      </el-form>
    ...
    <el-table 
      ...
      <el-table-column prop='no' fixed label='色样编号' width='110'>
        <template slot-scope='{row}'>
          <span class='c-link' @click='openSampleInfo(row)'>{{row.no}}</span>
        </template>
      </el-table-column>
      <el-table-column prop='status' fixed label='打样状态' width='110' align='center'>
        <template slot-scope='{row}'>
          <status-tag :status='row.status'/>
        </template>
      </el-table-column>
      <el-table-column prop='color_no' label='色板色号' width='110' show-overflow-tooltip />
      ...
      <el-table-column prop='other' label='其它要求' show-overflow-tooltip />
...
import applyListDialog from '../query/applyListDialog'
import sampleInfo from '../edit/info'

export default {
  components:{ statusTag, applyListDialog, sampleInfo },
  props:{
    size:{
      type:String,
      default:''
