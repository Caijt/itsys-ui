...
<template>
  <div>
    ...
    <el-table 
      ...
      <el-table-column v-if='!hideSubmitField' prop='status' label='打样状态' width='110' align='center'>
        <template slot-scope='{row}'>
      ...
      <el-table-column v-if='!hideSubmitField' prop='apply_total' label='打样次数' width='80' align='center'>
        <template slot-scope='{row}'>
      ...
      <el-table-column v-if='!hideSubmitField' prop='submit_user_name' label='提交人' />
...
export default {
  components:{ statusTag, applyListDialog },
  props:{
    size:{
      type:String,
      default:''
