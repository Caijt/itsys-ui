...
<template>
  <div>
    ...
    <el-table 
      ...
      <el-table-column prop='attach_ids' label='附件' width='80' align='center'>
        <template slot-scope='{row}'>
          <span class='c-link' @click='$refs.attachListDialog.open({attach_ids:row.attach_ids})'>{{ getAttachTotal(row.attach_ids)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop='create_user_name' label='提交人' width='90'/>
      <el-table-column prop='create_time' label='提交时间' sortable='custom' width='120'>
        <template slot-scope='{row}'>
    ...
    <attach-list-dialog :in-dialog='inDialog' ref='attachListDialog'/>
  </div>
</template>
<script>
...
import attachListDialog from '@/components/common/attach/listDialog'

export default {
  components:{ projectDetails, orderInfo, productList, attachListDialog },
  props:{
    size:{
      type:String,
      default:''
  ...
  methods:{
    ...
    getData() {
      this.loading=true
      colorSampleSupplyApi.getList(this.requestParams).then(res=>{
    ...
    del({ row,$index }){
      let confirmText = '确定删除此色样申请吗？'
      this.$confirm(confirmText,'提示',{
        type: 'warning'
      }).then(()=>{
        colorSampleApi.del(row.id).then(res=>{
    ...
    getAttachTotal(attach_ids){
      if(attach_ids){
        return attach_ids.split(',').length
      }else{
        return 0
      }
    }
