...
<template>
  <div>
    ...
    <el-table 
      ...
      <el-table-column prop='purchase_type' label='采购类型' show-overflow-tooltip />
      <el-table-column prop='purchase_oa' label='采购OA' show-overflow-tooltip />
...
export default {
  components:{ projectDetails,orderInfo  },
  props:{
    size:{
      type:String,
      default:''
  ...
  methods:{
    ...
    getData() {
      this.loading=true
      purchaseApi.getSummaryListByOrder(this.requestParams).then(res=>{
