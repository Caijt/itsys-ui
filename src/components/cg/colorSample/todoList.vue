...
<template>
  <div>
    ...
    <el-table 
      ...
      <el-table-column prop='file_date' label='归档日期' width='120' show-overflow-tooltip />
      <el-table-column prop='file_remarks' label='归档说明' width='120' show-overflow-tooltip />
      <el-table-column prop='create_user_name' label='销售申请人' width='100'/>      
      <el-table-column prop='submit_time' label='销售提交时间' sortable='custom' width='140'>
        <template slot-scope='{row}'>
          {{ $commonJs.formatDate(row.submit_time) }}
        </template>
      </el-table-column>
      <el-table-column prop='file_user_name' label='归档人' width='100'/>      
      <el-table-column prop='file_time' label='归档提交时间' sortable='custom' width='140'>
        <template slot-scope='{row}'>
...
export default {
  ...
  methods:{
    ...
    getData() {
      this.loading=true
      colorSampleApi.getTodoList(this.requestParams).then(res=>{
