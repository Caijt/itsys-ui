<template>
    ...
    >
        <div v-loading="loading" style="position: relative;">
            <el-alert
                class="no-print"
                title="为避免打印格式出现异常，请使用Firefox（火狐）浏览器打印"
                type="success"
            ...
            >
                ...
                >选择</el-checkbox>
                <div>
                    <div style="font-size:22px;text-align: center;font-weight: bold">IT资产交还单</div>
                    <div style="margin: 5px 0px">
                        <div style="display: flex">
                            <div style="flex:1">交还编号：{{item.no}}</div>
                    ...
                    <table class="c-table" style="border:2px solid #000">
                        <tr class="label">
                            ...
                            <td>交还部门</td>
                            <td>交还人</td>
                            <td style="width:60px">交还数量</td>
                        </tr>
                        <tr v-for="(a,index) in item.assets" :key="a.id">
        ...
        <span slot="footer">
            <el-tooltip content="开启后，可选择个别标签进行单独打印">
                <el-checkbox v-model="selectMode" border style="margin-right:10px">选择模式</el-checkbox>
            </el-tooltip>
...
export default {
    ...
    methods: {
        ...
        getData(params) {
            this.loading = true;
            assetApi.getPrintList({ ...params }).then(res => {
                this.loading = false;
                this.list = res.data;
            });
            return this;
