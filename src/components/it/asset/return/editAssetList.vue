<template>
    <div>
        <el-form :model="form" ref="form" class="c-form-mini" status-icon>
                ...
                <el-table-column prop="payment_price" label="交还数量" width="100" fixed="right">
                    <template slot-scope="{row}">
                        <el-form-item
                            ...
                            prop="return_amount"
                            :rules="getAmountRules(row)"
                        >
                            <el-input v-model.number="row.return_amount" :placeholder="row.used"></el-input>
                        </el-form-item>
                    </template>
                </el-table-column>
                <!-- slot[column] -->
...
export default {
    ...
    methods: {
        ...
        getData() {
            this.loading = true;
            paymentProjectApi
                .getList({ ...this.requestParams, ...this.initParams })
                .then(res => {
        ...
        openSelectAssetDiaog() {
            let selectedIds = this.list.map(item => {
                return item.record_item_id;
            });
            this.$refs.assetUseStatusListDialog.open().then(that => {
                that.$refs.list.initData({
                    not_item_ids: selectedIds
                });
            });
        },
        selectAsset(row) {
            let selectionList = this.$refs.assetUseStatusListDialog.$refs.list
                .selectionList;
